...
| Cliente JS (api-client, state, view, app) | Apoyo para construir los 4 módulos siguiendo la arquitectura objetivo del enunciado (`BoardApp` → `BoardApiClient` + `BoardState` + `BoardView`) y para mantener el estado inmutable de forma consistente | Cuatro módulos ES Modules con SVG nativo | Probamos el flujo completo en navegador real (crear, cargar, agregar, mover, conectar, guardar, recargar, error/retry) y verificamos con búsqueda de texto que `fetch` solo exista en `board-api-client.js` | El enunciado indicaba una ruta incorrecta para `index.html`; se corrigió a `static/index.html` |
...
| Prueba manual end-to-end | No se usó IA | Flujo completo verificado | Ejecutamos nosotros, en navegador, crear, cargar, agregar, mover, conectar, guardar/recargar y error/retry | — |
